...
@
\begin{description}
  \item[ggplot()] The main function. Can specify the dataset and variables for \textbf{all geoms} here.
    \item[geom] A geometric object: \texttt{geom\_point, geom\_line, geom\_text, geom\_violin}
    \item[aes] aesthetics: shape, transparency (alpha), color, fill, linetype.
    \item[scale] How is the data displayed (log, continuous, discrete, date)
\end{description}
\pause
\vspace{1em}
...
\begin{frame}[fragile]
\frametitle{Scatterplot (=Cont. \times Cont.)}
The most basic plot.
<<fig.width=7, out.width='.75\\linewidth'>>=
ggplot(frogs) +
